...
/**
 *  @brief Do UAP_IOCTL for wacp_mode configuration
 *
 *  @param wacp_data   WACP mode configuration
 *  @return            UAP_SUCCESS/UAP_FAILURE
 */
int uap_ioctl_wacp_mode(t_u8 *wacp_data)
{
	struct ifreq ifr;
	t_s32 sockfd;
	...
	memset(&ifr, 0, sizeof(ifr));
	strncpy(ifr.ifr_ifrn.ifrn_name, dev_name, strlen(dev_name));
	ifr.ifr_ifru.ifru_data = (void *)wacp_data;
	/* Perform ioctl */
	errno = 0;
	if (ioctl(sockfd, UAP_IOCTL_CMD, &ifr)) {
		...
		if (cmd_buf->result == CMD_SUCCESS) {
			...
			while ((unsigned int)size >= sizeof(tlvbuf_header)) {
				...
				case 1:
					...
					break;
				case 2:
					memcpy(&byte2, &tlv->data[0], oid_size);
					printf("%d",
					       (unsigned int)uap_le16_to_cpu(
						       byte2));
					break;
				case 4:
					memcpy(&byte4, &tlv->data[0], oid_size);
					printf("%d",
...
static command_table ap_command[] = {
	...
	{"block_frame", apcmd_sys_cfg_block_frame,
	 "\tSet/Get flag of block frame."},
	{NULL, NULL, 0}};
...
int is_input_valid(valid_inputs cmd, int argc, char *argv[])
{
	...
	case AP_BLOCK_FRAME:
		if (argc != 1) {
			printf("ERR:Incorrect number of arguments\n");
			ret = UAP_FAILURE;
		} else if (IS_HEX_OR_DIGIT(argv[0]) == UAP_FAILURE) {
			printf("ERR: Parameter must be a number.\n");
			ret = UAP_FAILURE;
		}
		break;
	default:
