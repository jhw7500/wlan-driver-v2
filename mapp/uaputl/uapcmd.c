	...
	if (argc == 0) {
		...
		if (argc == 1) {
			...
			if (mode & BITMAP_CHANNEL_ABOVE) {
				...
				tlv->bandcfg.chanWidth = CHAN_BW_40MHZ;
			}
...
int apcmd_sys_cfg_wacp_mode(int argc, char *argv[])
{
	...
	/** Fill the params */
	memset(&wacp_config, 0, sizeof(wacp_mode_t));
	wacp_config.subcmd = UAP_WACP_MODE;
	wacp_config.wacp_mode_tlv.tag = MRVL_AP_WACP_MODE_TLV_ID;
	wacp_config.wacp_mode_tlv.length = 1;
	if (argc == 0) {
		wacp_config.action = ACTION_GET;
	} else {
		wacp_config.action = ACTION_SET;
		wacp_config.wacp_mode_tlv.wacp_mode =
			(t_u8)A2HEXDECIMAL(argv[0]);
	}

	endian_convert_tlv_header_out(&wacp_config.wacp_mode_tlv);
	/* Send the UAP_IOCTL */
	if (UAP_FAILURE == uap_ioctl_wacp_mode((t_u8 *)&wacp_config)) {
		return UAP_FAILURE;
	}
	endian_convert_tlv_header_in(&wacp_config.wacp_mode_tlv);
...
/**
 *  @brief Show usage information for the sys_cfg_block_frame
 *   command
 *
 *  $return         N/A
 */
static void print_sys_cfg_block_frame_usage(void)
{
	printf("\nUsage : block_frame [flag]\n");
	printf("\nIf flag is provided, a 'set' is performed, else a 'get' is performed.");
	printf("\nflag: 0     - disable blocking frame.");
	printf("\n      0x800 - block authentication frame.");
	printf("\n");
	return;
}

/**
 *  @brief Control blocking AUTH frame
 *
 *   Usage: "block_frame [flag]"
 *
 *   Options: 0     - Disable
 *            0x800 - Block authentication frame
 *            empty - Get current setting
 *
 *
 *  @param argc     Number of arguments
 *  @param argv     Pointer to the arguments
 *  @return         UAP_SUCCESS/UAP_FAILURE
 */
int apcmd_sys_cfg_block_frame(int argc, char *argv[])
{
	apcmdbuf_sys_configure *cmd_buf = NULL;
	tlvbuf_block_frame *tlv = NULL;
	t_u8 *buffer = NULL;
	t_u16 cmd_len = 0;
	t_u16 buf_len = MRVDRV_SIZE_OF_CMD_BUFFER;
	int ret = UAP_SUCCESS;
	...
	while ((opt = getopt_long(argc, argv, "+", cmd_options, NULL)) != -1) {
		switch (opt) {
		default:
			print_sys_cfg_block_frame_usage();
			return UAP_SUCCESS;
		}
	}
	argc -= optind;
	...
	/* Check arguments */
	if (argc) {
		if (is_input_valid(AP_BLOCK_FRAME, argc, argv) != UAP_SUCCESS) {
			printf("ERR: Invalid input\n");
			return UAP_FAILURE;
		}
	}
	...
	/* Initialize the command length */
	cmd_len = sizeof(apcmdbuf_sys_configure) + sizeof(tlvbuf_block_frame);
	...
	/* Locate headers */
	cmd_buf = (apcmdbuf_sys_configure *)buffer;
	tlv = (tlvbuf_block_frame *)(buffer + sizeof(apcmdbuf_sys_configure));
	...
	cmd_buf->seq_num = 0;
	cmd_buf->result = 0;
	tlv->tag = NXP_BLOCK_FRAME_TLV_ID;
	tlv->length = 4;
	if (argc == 0) {
		cmd_buf->action = ACTION_GET;
	} else {
		cmd_buf->action = ACTION_SET;
		tlv->flag = (t_u32)A2HEXDECIMAL(argv[0]);
	}
	endian_convert_tlv_header_out(tlv);
	tlv->flag = uap_cpu_to_le16(tlv->flag);

	/* Send the command */
	ret = uap_ioctl((t_u8 *)cmd_buf, &cmd_len, buf_len);
	endian_convert_tlv_header_in(tlv);
	tlv->flag = uap_le16_to_cpu(tlv->flag);
	...
	if (ret == UAP_SUCCESS) {
		...
		if (cmd_buf->result == CMD_SUCCESS) {
			if (argc == 0) {
				printf("Block frame flag = 0x%x\n", tlv->flag);
			} else {
				printf("Block frame setting successful\n");
			}
		} else {
			printf("ERR:Could not %s!\n",
			       (argc == 0 ? "get" : "set"));
			ret = UAP_FAILURE;
		}
	} else {
