...
#define TYPE_EQ_MAGIC                                                          \
	(MAX_OPERAND + 6) /**< magic pkt:    =m operator for magic packet */
...
static int str2bin(char *str, mstack_t *stack)
{
	...
	while ((opstr = getop(str, &first_time)) != NULL) {
		if (isdigit((unsigned char)*opstr)) {
			...
			if (!push(stack, &op)) {
				printf("push byte magic operator failed\n");
				ret = MLAN_STATUS_FAILURE;
				break;
			}
		} else {
...
#define FILTER_MAGIC TYPE_EQ_MAGIC /**< byte sequence for magic byte */
