...
static int process_tsp(int argc, char *argv[]);
...
static int process_plinkstats(int argc, char *argv[]);
...
struct command_node command_list[] = {
	...
	{"ratebitmapcfg", process_ratebitmapcfg},
	{"ratemaxcfg", process_ratemaxcfg},
	{"mcstiercfg", process_mcstiercfg},
...
static char *usage[] = {
	...
	"         miracastcfg",
#endif
	"         mgmtframectrl", "         mgmtframetx", "         netmon",
#ifdef WIFI_DIRECT_SUPPORT
	"         cfg_noa", "         cfg_opp_ps",
#endif
	...
	"         twt_teardown", "         twt_report", "         rx_abort_cfg",
	...
	"         hal_phy_cfg", "         ips_cfg", "         getchload",
	"         ftm", "         getuuid", "         crosssynch",
	"         wlan_tsp_cfg",
...
static int process_getlog(int argc, char *argv[])
{
	...
	if (argc == 4 && !(strcmp(argv[3], "ext"))) {
		printf("rxStuckIssueCount-1                %u\n"
		       ...
		       stats->g_reset_rx_mac_cnt, stats->SdmaStuckCnt,
		       stats->dwCtlErrCnt, stats->dwBcnErrCnt,
		       stats->dwMgtErrCnt, stats->dwDatErrCnt,
		       stats->bigtk_mmeGoodCnt, stats->bigtk_replayErrCnt,
		       stats->bigtk_micErrCnt, stats->bigtk_mmeNotFoundCnt,
		       stats->currTemp, (stats->TXpwrMethod ? "OLPC" : "CLPC"),
		       ((stats->isDPDdone == 0xFF) ?
				"INVALID" :
				(stats->isDPDdone ? "YES" : "NO")));
	}
	...
	if (stats->cca_cnt_us != 0) {
		printf("cca_cnt_us                           %llu\n"
		       "rxAirtime_us                         %llu\n"
		       "txAirtime_us                         %llu\n",
		       stats->cca_cnt_us, stats->rxAirtime_us,
		       stats->txAirtime_us);
	}
...
/**
 *  @brief 	  Get fw support band from driver
 *
 *  @param fw_band a pointer to return FW band
 *
 *  @return        MLAN_STATUS_SUCCESS--success, otherwise--fail
 */

int get_bandcfg(t_u32 *fw_band)
{
	t_u8 *buffer = NULL;
	struct eth_priv_cmd *cmd = NULL;
	...
	prepare_buffer(buffer, PRIV_CMD_BANDCFG, 0, NULL);
	...
	if (ioctl(sockfd, MLAN_ETH_PRIV, &ifr)) {
		perror("");
		fprintf(stderr, "get bandcfg failed\n");
		if (cmd)
			free(cmd);
		if (buffer)
			free(buffer);
 ...
 */
static t_u8 is_6g_sub_band_allowed(void)
{
	int ret = 0;
	return ret;
}
	...
	case 0xff:
		...
		ret = get_txpwrlimit(fp_raw, argv, 0x13, buffer,
				     sizeof(mlan_ds_misc_chan_trpc_cfg) +
					     strlen(argv[2]),
				     cmd);
		if (is_6g_sub_band_allowed()) {
			ret = get_txpwrlimit(
				fp_raw, argv, 0x20, buffer,
				sizeof(mlan_ds_misc_chan_trpc_cfg) +
					strlen(argv[2]),
				cmd);
			ret = get_txpwrlimit(
				fp_raw, argv, 0x21, buffer,
				sizeof(mlan_ds_misc_chan_trpc_cfg) +
					strlen(argv[2]),
				cmd);
			ret = get_txpwrlimit(
				fp_raw, argv, 0x22, buffer,
				sizeof(mlan_ds_misc_chan_trpc_cfg) +
					strlen(argv[2]),
				cmd);
			ret = get_txpwrlimit(
				fp_raw, argv, 0x23, buffer,
				sizeof(mlan_ds_misc_chan_trpc_cfg) +
					strlen(argv[2]),
				cmd);
			ret = get_txpwrlimit(
				fp_raw, argv, 0x24, buffer,
				sizeof(mlan_ds_misc_chan_trpc_cfg) +
					strlen(argv[2]),
				cmd);
			ret = get_txpwrlimit(
				fp_raw, argv, 0x25, buffer,
				sizeof(mlan_ds_misc_chan_trpc_cfg) +
					strlen(argv[2]),
				cmd);
			ret = get_txpwrlimit(
				fp_raw, argv, 0x26, buffer,
				sizeof(mlan_ds_misc_chan_trpc_cfg) +
					strlen(argv[2]),
				cmd);
			ret = get_txpwrlimit(
				fp_raw, argv, 0x27, buffer,
				sizeof(mlan_ds_misc_chan_trpc_cfg) +
					strlen(argv[2]),
				cmd);
		}
...
static int process_mgmtframetx(int argc, char *argv[])
{
	...
	char *args[MAX_ARG_SIZE], *pos = NULL, mac_addr[21] = {0};
	t_u8 peer_mac[ETH_ALEN];
	...
	/* Parse file and process */
	while (config_get_line(line, MAX_CONFIG_LINE, config_file, &li, &pos)) {
		arg_num = parse_line(line, args, MAX_ARG_SIZE);
...
static int process_cwmode(int argc, char *argv[])
{
	...
	memcpy((char *)pos, CMD_NXP, strlen(CMD_NXP));
	pos += (strlen(CMD_NXP));
	strncpy((char *)pos, argv[2], BUFFER_LENGTH);
	pos += (strlen(argv[2]));
	action = pos;
	pos++;
...
static int process_plinkstats(int argc, char *argv[])
{
	...
	if (ioctl(sockfd, MLAN_ETH_PRIV, &ifr)) {
		perror("mlanutl");
		fprintf(stderr, "mlanutl: print link stats fail\n");
		ret = MLAN_STATUS_FAILURE;
		goto done;
	}
	...
	if ((argc == 3)) {
		if (data[0] == 1) {
			printf("Print linkstats: feature ENABLED\n");
			printf("  filter mask   = 0x%08x\n", data[1]);
			printf("  interval (s)  = %u\n", data[2]);
			printf("  netlink event = %d\n", data[3]);
		} else {
			printf("Print linkstats: feature DISABLED\n");
		}
		goto done;
	}
	...
	case 1: /* enable */
		...
		break;
	case 2: /* reset */
		printf("Print linkstats: statistics RESET\n");
		break;
	case 3: /* get */
	{
		struct eth_priv_linkstats *stats = NULL;
		pos += sizeof(data);
		stats = (struct eth_priv_linkstats *)pos;

		printf("Print linkstats: GET link statistics\n");
		printf("  Num TX Packets      : %lu\n", stats->tx_packets);
		printf("  Num TX Bytes        : %lu\n", stats->tx_bytes);
		printf("  Num RX Packets      : %lu\n", stats->rx_packets);
		printf("  Num RX Bytes        : %lu\n", stats->rx_bytes);
		...
		printf("  RTS Fail Count      : %lu\n", stats->rtsfailure);
		printf("  FCS Error Count     : %lu\n", stats->fcserror);
		printf("  TX Frame Count      : %lu\n", stats->txframe);
		printf("  Dropped Packets     : %lu\n", stats->tx_dropped);
		printf("  Num Deauth TX       : %lu\n",
		       stats->num_evt_deauth_tx);
		printf("  Num Deauth RX       : %lu\n",
		       stats->num_evt_deauth_rx);
		printf("  Associated STA List : \n");

		/* add per-STA list */
		char *getstalist_argv[] = {"mlanutl", dev_name, "getstalist"};
		int getstalist_argc =
			sizeof(getstalist_argv) / sizeof(getstalist_argv[0]);
		ret = process_getstalist(getstalist_argc, getstalist_argv);
		break;
	}
...
static int process_ctrldeauth(int argc, char *argv[])
{
	...
	/* dev_name contains the interface name. Checking whether
	 * mlan string is present in the interface name.
	 */
	is_sta_interface = !(strstr(dev_name, "mlan") == NULL);
	/* Fill up buffer */
#ifdef USERSPACE_32BIT_OVER_KERNEL_64BIT
	memset(cmd, 0, sizeof(struct eth_priv_cmd));
	memcpy(&cmd->buf, &buffer, sizeof(buffer));
	...
	/* For STA interface, only value 2 is valid.
	 * For UAP interface, only value 1 is valid.
	 */
	if ((is_sta_interface && buffer[0] == 2) ||
	    (!is_sta_interface && buffer[0] == 1))
		printf("enabled.\n");
	else
		printf("disabled.\n");
...
/**
 *  @brief Process rate bitmap config set/get
 *
 *  Usage:
 *    GET: mlanutl mlan0 ratebitmapcfg
 *    SET: mlanutl mlan0 ratebitmapcfg <hr_dsss> <ofdm> <ht0~7> <vht0~7> <he0~7>
 *         (26 hex values total)
 *
 *  @param argc   Number of arguments
 *  @param argv   A pointer to arguments array
...
static int process_ratebitmapcfg(int argc, char *argv[])
{
	...
	/* Check arguments: GET=3, SET=3+26=29 */
	if (argc != 3 && argc != 29) {
		printf("ERR: Invalid number of arguments.\n");
		printf("Usage:\n");
		printf("  GET: mlanutl mlan0 ratebitmapcfg\n");
		printf("  SET: mlanutl mlan0 ratebitmapcfg "
		       "<hr_dsss> <ofdm> <ht0~7> <vht0~7> <he0~7>\n");
		printf("       (26 hex/decimal values)\n");
		return MLAN_STATUS_FAILURE;
	}
	...
	if (ioctl(sockfd, MLAN_ETH_PRIV, &ifr)) {
		perror("mlanutl");
		fprintf(stderr, "mlanutl: ratebitmapcfg fail\n");
		if (cmd)
			free(cmd);
		if (buffer)
			free(buffer);
	...
	/* Process result */
	data = (int *)buffer;
	printf("Rate Bitmap Configuration:\n");
	printf("  [0]     HR/DSSS        : 0x%04X\n", data[0] & 0xFFFF);
	printf("  [1]     OFDM           : 0x%04X\n", data[1] & 0xFFFF);
	for (i = 0; i < 8; i++)
		printf("  [%2d]    HT MCS%d       : 0x%04X\n",
		       2 + i, i, data[2 + i] & 0xFFFF);
	for (i = 0; i < 8; i++)
		printf("  [%2d]    VHT MCS NSS%d  : 0x%04X\n",
		       10 + i, i + 1, data[10 + i] & 0xFFFF);
	for (i = 0; i < 8; i++)
		printf("  [%2d]    HE MCS NSS%d   : 0x%04X\n",
		       18 + i, i + 1, data[18 + i] & 0xFFFF);
...
/**
 *  @brief Helper: generate MCS bitmask for MCS 0..max_mcs
 *  @param max_mcs  Maximum MCS index (0-15)
 *  @return         Bitmask with bits 0..max_mcs set
 */
static t_u16 mcs_mask(int max_mcs)
{
	if (max_mcs < 0)
		return 0;
	if (max_mcs >= 15)
		return 0xFFFF;
	return (t_u16)((1 << (max_mcs + 1)) - 1);
}

/** Legacy OFDM rates in Mbps, indexed by bit position */
static const int ofdm_rates[] = {6, 9, 12, 18, 24, 36, 48, 54};
/** Legacy HR/DSSS rates in Mbps (x10 for 5.5), indexed by bit position */
static const int dsss_rates[] = {1, 2, 5, 11}; /* 5 = 5.5Mbps */

/**
 *  @brief Helper: generate Legacy OFDM+DSSS bitmask for max rate in Mbps
 *  @param max_rate  Maximum rate in Mbps (1,2,5,6,9,11,12,18,24,36,48,54)
 *  @param dsss_out  Output: HR/DSSS bitmap
 *  @param ofdm_out  Output: OFDM bitmap
 */
static void legacy_mask(int max_rate, t_u16 *dsss_out, t_u16 *ofdm_out)
{
	int i;
	*dsss_out = 0;
	*ofdm_out = 0;
	for (i = 0; i < 4; i++) {
		if (dsss_rates[i] <= max_rate)
			*dsss_out |= (1 << i);
	}
	for (i = 0; i < 8; i++) {
		if (ofdm_rates[i] <= max_rate)
			*ofdm_out |= (1 << i);
	}
}

#define MCS_TIER_NOT_SUPPORTED 0x3
#define VHT_MCS_TIER_0_7 0x0
#define VHT_MCS_TIER_0_8 0x1
#define VHT_MCS_TIER_0_9 0x2
#define HE_MCS_TIER_0_7 0x0
#define HE_MCS_TIER_0_9 0x1
#define HE_MCS_TIER_0_11 0x2

static int vht_max_to_tier(int max_mcs)
{
	if (max_mcs == 7)
		return VHT_MCS_TIER_0_7;
	if (max_mcs == 8)
		return VHT_MCS_TIER_0_8;
	if (max_mcs == 9)
		return VHT_MCS_TIER_0_9;
	return -1;
}

static int he_max_to_tier(int max_mcs)
{
	if (max_mcs == 7)
		return HE_MCS_TIER_0_7;
	if (max_mcs == 9)
		return HE_MCS_TIER_0_9;
	if (max_mcs == 11)
		return HE_MCS_TIER_0_11;
	return -1;
}

static int vht_tier_to_max(int tier)
{
	if (tier == VHT_MCS_TIER_0_7)
		return 7;
	if (tier == VHT_MCS_TIER_0_8)
		return 8;
	if (tier == VHT_MCS_TIER_0_9)
		return 9;
	return -1;
}

static int he_tier_to_max(int tier)
{
	if (tier == HE_MCS_TIER_0_7)
		return 7;
	if (tier == HE_MCS_TIER_0_9)
		return 9;
	if (tier == HE_MCS_TIER_0_11)
		return 11;
	return -1;
}

static int parse_he_band(const char *band)
{
	if (!band)
		return -1;
	if (!strcasecmp(band, "2g") || !strcasecmp(band, "2.4g"))
		return 0x01;
	if (!strcasecmp(band, "5g"))
		return 0x02;
	if (!strcasecmp(band, "both"))
		return 0x03;
	return -1;
}

static int is_decimal_arg(const char *arg)
{
	int i;
...
static const char *he_band_to_string(t_u8 band)
{
	if (band == 0x01)
		return "2G";
	if (band == 0x02)
...
static t_u16 update_supported_nss_map(t_u16 current_map, t_u16 tier_value)
{
	t_u16 updated_map = 0;
	int nss;

	for (nss = 1; nss <= 8; nss++) {
		t_u16 current = (current_map >> (2 * (nss - 1))) & 0x3;
		t_u16 value =
			(current == MCS_TIER_NOT_SUPPORTED) ? MCS_TIER_NOT_SUPPORTED :
									     tier_value;
		updated_map |= (value & 0x3) << (2 * (nss - 1));
	}
...
static int get_sta_vht_assoc_cfg(struct eth_priv_vhtcfg *vhtcfg_out)
{
	t_u8 *buffer = NULL;
	struct eth_priv_cmd *cmd = NULL;
	struct ifreq ifr;
	...
	prepare_buffer(buffer, "vhtcfg", 2, args);
	cmd = (struct eth_priv_cmd *)malloc(sizeof(struct eth_priv_cmd));
	if (!cmd) {
		printf("ERR:Cannot allocate buffer!\n");
		free(buffer);
	...
	if (ioctl(sockfd, MLAN_ETH_PRIV, &ifr)) {
		perror("mlanutl");
		fprintf(stderr, "mlanutl: mcstiercfg vhtcfg GET fail\n");
		goto done;
	}

	num = *buffer;
	if (!num) {
		fprintf(stderr, "mlanutl: mcstiercfg no VHT config returned\n");
		goto done;
	}

	memcpy(vhtcfg_out, buffer + 1, sizeof(*vhtcfg_out));
	ret = MLAN_STATUS_SUCCESS;
...
static int set_sta_vht_assoc_cfg(struct eth_priv_vhtcfg *vhtcfg)
{
	t_u8 *buffer = NULL;
	struct eth_priv_cmd *cmd = NULL;
	struct ifreq ifr;
	char band[12], txrx[12], bwcfg[12], vhtcap[16], tx_mcs[16], rx_mcs[16];
	char *args[] = {band, txrx, bwcfg, vhtcap, tx_mcs, rx_mcs};
	int ret = MLAN_STATUS_FAILURE;
	...
	snprintf(band, sizeof(band), "%u", (unsigned int)vhtcfg->band);
	snprintf(txrx, sizeof(txrx), "%u", (unsigned int)vhtcfg->txrx);
	snprintf(bwcfg, sizeof(bwcfg), "%u", (unsigned int)vhtcfg->bwcfg);
	snprintf(vhtcap, sizeof(vhtcap), "0x%08X",
		 (unsigned int)vhtcfg->vht_cap_info);
	snprintf(tx_mcs, sizeof(tx_mcs), "0x%04X",
		 (unsigned int)(vhtcfg->vht_tx_mcs & 0xFFFF));
	snprintf(rx_mcs, sizeof(rx_mcs), "0x%04X",
		 (unsigned int)(vhtcfg->vht_rx_mcs & 0xFFFF));

	prepare_buffer(buffer, "vhtcfg", 6, args);
	cmd = (struct eth_priv_cmd *)malloc(sizeof(struct eth_priv_cmd));
	if (!cmd) {
		printf("ERR:Cannot allocate buffer!\n");
		free(buffer);
	...
	if (ioctl(sockfd, MLAN_ETH_PRIV, &ifr)) {
		perror("mlanutl");
		fprintf(stderr, "mlanutl: mcstiercfg vhtcfg SET fail\n");
		goto done;
	}
...
static int get_he_cfg(mlan_ds_11ax_he_cfg *hecfg_out)
{
	t_u8 *buffer = NULL;
	struct eth_priv_cmd *cmd = NULL;
	struct ifreq ifr;
	int cmd_header_len = strlen(CMD_NXP) + strlen("11axcfg");
	int ret = MLAN_STATUS_FAILURE;
	...
	prepare_buffer(buffer, "11axcfg", 0, NULL);
	cmd = (struct eth_priv_cmd *)malloc(sizeof(struct eth_priv_cmd));
	if (!cmd) {
		printf("ERR:Cannot allocate buffer!\n");
		free(buffer);
	...
	if (ioctl(sockfd, MLAN_ETH_PRIV, &ifr)) {
		perror("mlanutl: 11axcfg");
		fprintf(stderr, "mlanutl: mcstiercfg 11axcfg GET fail\n");
		goto done;
	}

	memcpy(hecfg_out, buffer + cmd_header_len, sizeof(*hecfg_out));
	ret = MLAN_STATUS_SUCCESS;
...
static int set_he_cfg(mlan_ds_11ax_he_cfg *hecfg)
{
	t_u8 *buffer = NULL;
	struct eth_priv_cmd *cmd = NULL;
	struct ifreq ifr;
	...
	prepare_buffer(buffer, "11axcfg", 0, NULL);
	memcpy(buffer + cmd_header_len, hecfg, sizeof(*hecfg));
	...
	cmd->buf = buffer;
#endif
	cmd->used_len = cmd_header_len + sizeof(*hecfg);
	cmd->total_len = BUFFER_LENGTH;
	...
	if (ioctl(sockfd, MLAN_ETH_PRIV, &ifr)) {
		perror("mlanutl: 11axcfg");
		fprintf(stderr, "mlanutl: mcstiercfg 11axcfg SET fail\n");
		goto done;
	}
...
static int get_ht_stream_mode(void)
{
	...
	prepare_buffer(buffer, "htstreamcfg", 0, NULL);
	cmd = (struct eth_priv_cmd *)malloc(sizeof(struct eth_priv_cmd));
	if (!cmd) {
		free(buffer);
		return -1;
...
static int set_ht_stream_mode(int mode)
{
	t_u8 *buffer = NULL;
	struct eth_priv_cmd *cmd = NULL;
	struct ifreq ifr;
	...
	snprintf(mode_str, sizeof(mode_str), "0x%02x", mode);
	prepare_buffer(buffer, "htstreamcfg", 1, args);
	cmd = (struct eth_priv_cmd *)malloc(sizeof(struct eth_priv_cmd));
	if (!cmd) {
		free(buffer);
		return MLAN_STATUS_FAILURE;
...
static int process_mcstiercfg(int argc, char *argv[])
{
	...
	for (i = 3; i < argc; i++) {
		if (!strcasecmp(argv[i], "reset")) {
			...
			set_he = TRUE;
			break;
		} else if (!strcasecmp(argv[i], "ht") && i + 1 < argc) {
			ht_max = atoi(argv[++i]);
			if (ht_max != 7 && ht_max != 15) {
				printf("ERR: HT tier supports only 7 (1x1) or 15 (2x2)\n");
				return MLAN_STATUS_FAILURE;
			}
			set_ht = TRUE;
		} else if (!strcasecmp(argv[i], "vht") && i + 1 < argc) {
			vht_max = atoi(argv[++i]);
			if (vht_max_to_tier(vht_max) < 0) {
				printf("ERR: VHT tier supports only 7, 8, 9\n");
				return MLAN_STATUS_FAILURE;
			}
			set_vht = TRUE;
		} else if (!strcasecmp(argv[i], "he") && i + 1 < argc) {
			if (parse_he_band(argv[i + 1]) >= 0) {
				he_band = parse_he_band(argv[++i]);
				if (i + 1 >= argc) {
					printf("ERR: HE tier requires a max MCS value\n");
					return MLAN_STATUS_FAILURE;
				}
			} else if (!is_decimal_arg(argv[i + 1])) {
				printf("ERR: HE band supports only 2g, 5g, both\n");
				return MLAN_STATUS_FAILURE;
			}
			he_max = atoi(argv[++i]);
			if (he_max_to_tier(he_max) < 0) {
				printf("ERR: HE tier supports only 7, 9, 11\n");
				return MLAN_STATUS_FAILURE;
			}
			set_he = TRUE;
		} else {
			...
			printf("Usage:\n");
			printf("  GET:   mlanutl mlan0 mcstiercfg\n");
			printf("  SET:   mlanutl mlan0 mcstiercfg [ht <7|15>] [vht <7|8|9>] [he [2g|5g|both] <7|9|11>]\n");
			printf("  RESET: mlanutl mlan0 mcstiercfg reset\n");
			return MLAN_STATUS_FAILURE;
		}
	}

	if (!set_ht && !set_vht && !set_he) {
		printf("ERR: No options specified\n");
		return MLAN_STATUS_FAILURE;
	}

	if (set_ht) {
		int mode = (ht_max <= 7) ? HT_STREAM_MODE_1X1 :
					   HT_STREAM_MODE_2X2;
		if (set_ht_stream_mode(mode) != MLAN_STATUS_SUCCESS) {
			printf("ERR: HT stream mode set failed\n");
			return MLAN_STATUS_FAILURE;
		}
		printf("Applied HT stream: %s (MCS 0~%d)\n",
	...
	if (set_vht) {
		...
		vhtcfg.vht_tx_mcs = update_supported_nss_map(
			...
			(t_u16)vht_max_to_tier(vht_max));
		vhtcfg.vht_rx_mcs = update_supported_nss_map(
	...
	if (set_he) {
		...
		he_rx_mcs = update_supported_nss_map(he_rx_mcs,
					      (t_u16)he_max_to_tier(he_max));
		he_tx_mcs = update_supported_nss_map(he_tx_mcs,
					      (t_u16)he_max_to_tier(he_max));
		hecfg.he_cap.he_txrx_mcs_support[0] = he_rx_mcs & 0xFF;
		hecfg.he_cap.he_txrx_mcs_support[1] = (he_rx_mcs >> 8) & 0xFF;
		hecfg.he_cap.he_txrx_mcs_support[2] = he_tx_mcs & 0xFF;
		hecfg.he_cap.he_txrx_mcs_support[3] = (he_tx_mcs >> 8) & 0xFF;
		if (set_he_cfg(&hecfg) != MLAN_STATUS_SUCCESS)
			return MLAN_STATUS_FAILURE;
...
/**
 *  @brief Process rate max MCS config — simplified rate bitmap control
 *
 *  Usage:
 *    GET:   mlanutl mlan0 ratemaxcfg
 *    SET:   mlanutl mlan0 ratemaxcfg [he <max>] [vht <max>] [ht <max>] [lg <max_mbps>]
 *    RESET: mlanutl mlan0 ratemaxcfg reset
 *
 *  Examples:
 *    mlanutl mlan0 ratemaxcfg he 8 vht 8 ht 7
 *    mlanutl mlan0 ratemaxcfg he 7
 *    mlanutl mlan0 ratemaxcfg reset
 *
 *  @param argc   Number of arguments
 *  @param argv   A pointer to arguments array
 *  @return       MLAN_STATUS_SUCCESS--success, otherwise--fail
...
static int process_ratemaxcfg(int argc, char *argv[])
{
	...
	int *data = NULL;
	int i;
	/* Default max MCS/rate: full range */
	int he_max = 11, vht_max = 9, ht_max = 7, lg_max = 54;
	int do_set = 0;
	char bitmap_args[26][12];
	char *new_argv[29]; /* mlanutl mlan0 ratebitmapcfg + 26 values */
	t_u16 bitmap[26];

	if (argc == 3) {
		/* GET: read current bitmap and display as max MCS */
		buffer = (t_u8 *)malloc(BUFFER_LENGTH);
		if (!buffer) {
			printf("ERR:Cannot allocate buffer!\n");
			return MLAN_STATUS_FAILURE;
		}
		prepare_buffer(buffer, "ratebitmapcfg", 0, NULL);
		cmd = (struct eth_priv_cmd *)malloc(
			sizeof(struct eth_priv_cmd));
		if (!cmd) {
			printf("ERR:Cannot allocate buffer!\n");
		...
		if (ioctl(sockfd, MLAN_ETH_PRIV, &ifr)) {
			perror("mlanutl");
			fprintf(stderr, "mlanutl: ratemaxcfg GET fail\n");
			free(cmd);
			free(buffer);
			return MLAN_STATUS_FAILURE;
		}
		...
		/* Find highest set bit for each standard */
		ht_max = -1;
		for (i = 15; i >= 0; i--) {
			if (data[2] & (1 << i)) {
				ht_max = i;
		...
		he_max = -1;
		for (i = 11; i >= 0; i--) {
			if ((data[18] | data[19]) & (1 << i)) {
				he_max = i;
				break;
			}
		}

		/* Find max legacy rate */
		lg_max = 0;
		for (i = 7; i >= 0; i--) {
			if (data[1] & (1 << i)) {
				lg_max = ofdm_rates[i];
		...
		printf("Rate Max Configuration:\n");
		printf("  LG  (Legacy): max %dMbps (DSSS=0x%04X, OFDM=0x%04X)\n",
		       lg_max, data[0] & 0xFFFF, data[1] & 0xFFFF);
		printf("  HT  (11n)   : MCS 0~%d\n", ht_max);
		printf("  VHT (11ac)  : MCS 0~%d  (NSS1=0x%04X, NSS2=0x%04X)\n",
		       vht_max, data[10] & 0xFFFF, data[11] & 0xFFFF);
		printf("  HE  (11ax)  : MCS 0~%d (NSS1=0x%04X, NSS2=0x%04X)\n",
		       he_max, data[18] & 0xFFFF, data[19] & 0xFFFF);
	...
	for (i = 3; i < argc; i++) {
		if (strcasecmp(argv[i], "reset") == 0) {
			...
			do_set = 1;
			break;
		} else if (strcasecmp(argv[i], "lg") == 0 && i + 1 < argc) {
			lg_max = atoi(argv[++i]);
			if (lg_max < 1 || lg_max > 54) {
				printf("ERR: Legacy rate range 1~54 (Mbps)\n");
				printf("  Valid: 1, 2, 5(5.5), 6, 9, 11, 12, 18, 24, 36, 48, 54\n");
				return MLAN_STATUS_FAILURE;
			}
			do_set = 1;
		} else if (strcasecmp(argv[i], "he") == 0 && i + 1 < argc) {
			he_max = atoi(argv[++i]);
			if (he_max < 0 || he_max > 11) {
				printf("ERR: HE MCS range 0~11\n");
				return MLAN_STATUS_FAILURE;
			}
			do_set = 1;
		} else if (strcasecmp(argv[i], "vht") == 0 && i + 1 < argc) {
			vht_max = atoi(argv[++i]);
			if (vht_max < 0 || vht_max > 9) {
				printf("ERR: VHT MCS range 0~9\n");
				return MLAN_STATUS_FAILURE;
			}
			do_set = 1;
		} else if (strcasecmp(argv[i], "ht") == 0 && i + 1 < argc) {
			ht_max = atoi(argv[++i]);
			if (ht_max < 0 || ht_max > 15) {
				printf("ERR: HT MCS range 0~15\n");
				return MLAN_STATUS_FAILURE;
			}
			do_set = 1;
		} else {
			printf("ERR: Unknown option '%s'\n", argv[i]);
			printf("Usage:\n");
			printf("  GET:   mlanutl mlan0 ratemaxcfg\n");
			printf("  SET:   mlanutl mlan0 ratemaxcfg [he <0~11>] [vht <0~9>] [ht <0~15>] [lg <1~54>]\n");
			printf("  RESET: mlanutl mlan0 ratemaxcfg reset\n");
			return MLAN_STATUS_FAILURE;
		}
	}
	...
	/* Build bitmap_rates[26] */
	memset(bitmap, 0, sizeof(bitmap));
	legacy_mask(lg_max, &bitmap[0], &bitmap[1]);
	bitmap[2] = mcs_mask(ht_max); /* HT NSS1 */
	bitmap[3] = mcs_mask(ht_max - 8); /* HT NSS2 (MCS 8~15) */
	bitmap[10] = mcs_mask(vht_max); /* VHT NSS1 */
	bitmap[11] = mcs_mask(vht_max); /* VHT NSS2 */
	bitmap[18] = mcs_mask(he_max); /* HE NSS1 */
	bitmap[19] = mcs_mask(he_max); /* HE NSS2 */

	/* Build argv for ratebitmapcfg: reuse existing ioctl path */
	new_argv[0] = argv[0];
	new_argv[1] = argv[1];
	new_argv[2] = "ratebitmapcfg";
	for (i = 0; i < 26; i++) {
		snprintf(bitmap_args[i], sizeof(bitmap_args[i]),
			 "0x%04X", bitmap[i]);
		new_argv[3 + i] = bitmap_args[i];
	}

	printf("Applying: LG max=%dMbps, HT max=MCS%d, VHT max=MCS%d, HE max=MCS%d\n",
	       lg_max, ht_max, vht_max, he_max);

	return process_ratebitmapcfg(29, new_argv);
}
...
/**
 *  @brief Process Set/Get TSP Config
 *  @param argc   Number of arguments
 *  @param argv   A pointer to arguments array
 *  @return     MLAN_STATUS_SUCCESS--success, otherwise--fail
 */
static int process_tsp(int argc, char *argv[])
{
	...
	if ((argc < 3) || (argc > 8 && argc != 11)) {
		fprintf(stderr, "mlanutl: Invalid number of arguments\n");
		fprintf(stderr,
			"Usage: \nmlanutl <interface> wlan_tsp_cfg [enable] [backoff] [high threshold] [low threshold] [duty_cyc_step] [duty_cyc_min] [high_thrshld_temp] [low_thrshld_temp]\n");
		return MLAN_STATUS_FAILURE;
	}
	...
	if (argc > 3) {
		if ((atoi(argv[3]) < 0) || (atoi(argv[3]) > 1)) {
			fprintf(stderr,
				"ERR: Invalid enable value <valid 0/1>\n");
			return MLAN_STATUS_FAILURE;
		}

		if ((atoi(argv[3]) != 0) && (argc != 11)) {
			fprintf(stderr,
				"mlanutl: for enabling TSP, please provide all other arguments as well\n");
			fprintf(stderr,
				"Usage: \nmlanutl <interface> wlan_tsp_cfg [enable] [backoff] [high threshold] [low threshold] [duty_cyc_step] [duty_cyc_min] [high_thrshld_temp] [low_thrshld_temp]\n");
			return MLAN_STATUS_FAILURE;
		}
	...
	if (ioctl(sockfd, MLAN_ETH_PRIV, &ifr)) {
		perror("mlanutl");
		fprintf(stderr, "mlanutl: TSP config fail\n");
		if (cmd)
			free(cmd);
		if (buffer)
			free(buffer);
	...
	/* Process result */
	if (argc == 3) {
		cfg = (mlan_ds_tsp_cfg *)buffer;
		/* Show TSP config data */
		printf("TSP Configuration:\n");
		printf("Enable:%d (%s)\n", cfg->enable,
		       (cfg->enable == 1) ?
			       "Enabled" :
			       ((cfg->enable == 0) ? "Disabled" :
						     "Invalid option set"));
		...
		if (cfg->enable == 1) {
			printf("\tPower backoff:%d\n\tHigh Power Threshold:%d\n\tLow Power Threshold:%d\n\tDuty Cycle Step:%d\n\tDuty CycleMin :%d\n\tHigh Threshold Temperature:%d\n\tLow Threshold Temperature:%d\n",
			       cfg->backoff, cfg->high_thrshld,
			       cfg->low_thrshld, cfg->duty_cyc_step,
			       cfg->duty_cyc_min, cfg->high_thrshld_temp,
			       cfg->low_thrshld_temp);
		}

		printf("\tCAU TSEN Temperature:%d\n", cfg->reg_cau_val);
		printf("\tRFU Temperature: \n");
		for (rfu = 0; rfu < MAX_RFUS; rfu++) {
			for (rpath = 0; (rpath < MAX_PATHS) &&
...
/**
 *  @brief Process auth_assoc_timeout_cfg configuration
 *  @param argc   Number of arguments
 *  @param argv   A pointer to arguments array
 *  @return     MLAN_STATUS_SUCCESS--success, otherwise--fail
 */
static int process_auth_assoc_timeout_cfg(int argc, char *argv[])
{
	...
	struct eth_priv_cmd *cmd = NULL;
	struct ifreq ifr;
	struct eth_priv_auth_assoc_timeout_cfg *timeout_cfg;
	...
	if (ioctl(sockfd, MLAN_ETH_PRIV, &ifr)) {
		perror("mlanutl");
		fprintf(stderr, "mlanutl: auth_assoc_timeout_cfg fail\n");
		if (cmd)
			free(cmd);
		if (buffer)
			free(buffer);
	...
	/* Process result */
	timeout_cfg = (struct eth_priv_auth_assoc_timeout_cfg *)buffer;
	/* Show auth assoc timeout configure */
	printf("Auth Assoc Timeout Configuration:\n");
	printf("    auth_timeout:                   %d ms\n",
	       timeout_cfg->auth_timeout);
	printf("    auth_retry_timeout_if_ack:      %d ms \n",
	       timeout_cfg->auth_retry_timeout_if_ack);
	printf("    auth_retry_timeout_if_no_ack:   %d ms\n",
	       timeout_cfg->auth_retry_timeout_if_no_ack);
	printf("    assoc_timeout:                  %d ms\n",
	       timeout_cfg->assoc_timeout);
	printf("    reassoc_timeout:                %d ms\n",
	       timeout_cfg->reassoc_timeout);
	printf("    retry_timeout:                  %d ms\n",
	       timeout_cfg->retry_timeout);
...
#ifdef UAP_SUPPORT
/**
 *  @brief Prepare agcs command buffer
 *  @param agcs_req pointer to wlan_ioctl_agcs_info structure
 *  @param num      Number of arguments
 *  @param args     Arguments list
 *  @return         MLAN_STATUS_SUCCESS
 */
static int prepare_agcs_buffer(wlan_ioctl_agcs_info *agcs_req, t_u32 num,
			       char *args[])
{
	int arg_idx = 0;
	char *parg_tok = NULL;
	...
	for (arg_idx = 0; arg_idx < (int)num; arg_idx++) {
		if (strncmp(args[arg_idx], "mode=", strlen("mode=")) == 0) {
			/* "mode" token string handler */
			agcs_req->mode = strtol(args[arg_idx] + strlen("mode="),
						NULL, 16);
			if ((agcs_req->mode & 0xF) && !(agcs_req->mode & 0x1)) {
				printf("ERR: Not support this mode without set BIT0 on\n");
				return MLAN_STATUS_FAILURE;
			}
		} else if (strncmp(args[arg_idx],
				   "avg_th=", strlen("avg_th=")) == 0) {
			/* "avg_th" token string handler */
			agcs_req->avg_threshold_percentage =
				atoi(args[arg_idx] + strlen("avg_th="));
		} else if (strncmp(args[arg_idx],
				   "rx_min_pkt=", strlen("rx_min_pkt=")) == 0) {
			/* "rx_min_pkt" token string handler */
			agcs_req->rx_min_pkt_count =
				atoi(args[arg_idx] + strlen("rx_min_pkt="));
		} else if (strncmp(args[arg_idx],
				   "tx_min_pkt=", strlen("tx_min_pkt=")) == 0) {
			/* "tx_min_pkt" token string handler */
			agcs_req->tx_min_pkt_count =
				atoi(args[arg_idx] + strlen("tx_min_pkt="));
		} else if (strncmp(args[arg_idx], "sample_time=",
				   strlen("sample_time=")) == 0) {
			/* "sample_time" token string handler */
			agcs_req->sample_time =
				atoi(args[arg_idx] + strlen("sample_time="));
		} else if (strncmp(args[arg_idx], "sample_window=",
				   strlen("sample_window=")) == 0) {
			/* "sample_window" token string handler */
			agcs_req->sample_count_window =
				atoi(args[arg_idx] + strlen("sample_window="));
		} else if (strncmp(args[arg_idx],
				   "hit_cnt=", strlen("hit_cnt=")) == 0) {
			/* "hit_cnt" token string handler */
			agcs_req->continuous_hit_count =
				atoi(args[arg_idx] + strlen("hit_cnt="));
		} else if (strncmp(args[arg_idx],
				   "nf_margin=", strlen("nf_margin=")) == 0) {
			/* "nf_margin" token string handler */
			agcs_req->nf_margin =
				atoi(args[arg_idx] + strlen("nf_margin="));
		} else if (strncmp(args[arg_idx],
				   "chload_th=", strlen("chload_th=")) == 0) {
			/* "chload_th" token string handler */
			agcs_req->chload_threshold_percentage =
				atoi(args[arg_idx] + strlen("chload_th="));
		} else if (strncmp(args[arg_idx],
				   "csa_cnt=", strlen("csa_cnt=")) == 0) {
			/* "csa_cnt" token string handler */
			agcs_req->csa_cnt =
				atoi(args[arg_idx] + strlen("csa_cnt="));
		} else if (strncmp(args[arg_idx], "chan=", strlen("chan=")) ==
			   0) {
			/* "chan" token string handler */
			parg_tok = args[arg_idx] + strlen("chan=");
			...
			while ((parg_tok = strtok_r(parg_tok, ",",
				...
				*pscratch = 0;
				...
				if (((agcs_req->chan_list[chan_cmd_idx]
					      .chan_number > MAX_CHAN_BG_BAND) &&
				     !agcs_req->chan_list[chan_cmd_idx]
					      .radio_type) ||
				    ((agcs_req->chan_list[chan_cmd_idx]
				...
				agcs_req->chan_list[chan_cmd_idx].scan_time =
					30;
				chan_cmd_idx++;
			}
		} else if (strncmp(args[arg_idx],
				   "scan_dur=", strlen("scan_dur=")) == 0) {
			/* "scan_dur" token string handler */
			scan_time = atoi(args[arg_idx] + strlen("scan_dur="));
			agcs_req->chan_list[0].scan_time = scan_time;
		} else if (strncmp(args[arg_idx], "nav_mitigation_th=",
				   strlen("nav_mitigation_th=")) == 0) {
			/* "nav_mitigation_th" token string handler */
			agcs_req->nav_mitigation_th = atoi(
				args[arg_idx] + strlen("nav_mitigation_th="));
		} else if (strncmp(args[arg_idx], "ch_th=", strlen("ch_th=")) ==
			   0) {
			/* "ch_th" token string handler */
			agcs_req->ch_th =
				atoi(args[arg_idx] + strlen("ch_th="));
		} else if (strncmp(args[arg_idx], "min_pkt_percentage=",
				   strlen("min_pkt_percentage=")) == 0) {
			/* "min_pkt_percentage" token string handler */
			agcs_req->min_pkt_percentage = atoi(
				args[arg_idx] + strlen("min_pkt_percentage="));
		} else {
	...
	/* Update all the channels to have the same scan time */
	for (tmp_idx = 1; tmp_idx < chan_cmd_idx; tmp_idx++) {
		agcs_req->chan_list[tmp_idx].scan_time = scan_time;
	}
...
/**
 *  @brief Show usage information for the agcs command
 *
 *  $return         N/A
 */
static void print_agcs_usage(void)
{
	printf("Invalid input command provided to agcs! \n");
	printf("Usage: ./mlanutl uap0 agcs [mode=<bit_map>] [avg_th=<value>] [rx_min_pkt=<value>] [tx_min_pkt=<value>] ...\n");
	printf("[ch_th=<value>] only used for agcs only per ch load on NH\n");
	printf("[avg_th=<value>][rx_min_pkt=<value>][tx_min_pkt=<value>][sample_window=<value>][hit_cnt=<value>][nav_mitigation_th=<value>][min_pkt_percentage=<value>] not used for agcs only per ch load on NH\n");
	printf("To SET: ./mlanutl uap0 agcs mode=0x1\n");
	printf("To GET: ./mlanutl uap0 agcs\n");
	printf("\n");
}

/**
 *  @brief Process agcs configuration
 *  @param argc   Number of arguments
 *  @param argv   A pointer to arguments array
 *  @return     MLAN_STATUS_SUCCESS--success, otherwise--fail
 */
static int process_agcs(int argc, char *argv[])
{
	wlan_ioctl_agcs_info *agcs_req = NULL;
	t_u8 *pos = NULL;
	t_u8 *buffer = NULL;
	struct eth_priv_cmd *cmd = NULL;
	struct ifreq ifr;
	...
	/* Insert arguments */
	agcs_req = (wlan_ioctl_agcs_info *)pos;

	if (prepare_agcs_buffer(agcs_req, (argc - 3), &argv[3])) {
		print_agcs_usage();
		free(buffer);
		return MLAN_STATUS_FAILURE;
	}
	if (argc == 3) {
		agcs_req->action = MLAN_ACT_GET;
	} else {
		agcs_req->action = MLAN_ACT_SET;
	}
	...
	if (ioctl(sockfd, MLAN_ETH_PRIV, &ifr)) {
		perror("mlanutl");
		fprintf(stderr, "mlanutl: agcs fail\n");
		if (cmd)
			free(cmd);
		if (buffer)
			free(buffer);
	...
	if (argc == 3) {
		/* GET operation */
		agcs_req = (wlan_ioctl_agcs_info *)buffer;
		printf("agcs param: mode=%d avg_th=%d rx_min_pkt=%d tx_min_pkt=%d sample_time=%d sample_window=%d hit_cnt=%d nf_margin=%d chload_th=%d csa_cnt=%d nav_mitigation_th=%d ch_th=%d min_pkt_percentage=%d\n\n",
		       agcs_req->mode, agcs_req->avg_threshold_percentage,
		       agcs_req->rx_min_pkt_count, agcs_req->tx_min_pkt_count,
		       agcs_req->sample_time, agcs_req->sample_count_window,
		       agcs_req->continuous_hit_count, agcs_req->nf_margin,
		       agcs_req->chload_threshold_percentage, agcs_req->csa_cnt,
		       agcs_req->nav_mitigation_th, agcs_req->ch_th,
		       agcs_req->min_pkt_percentage);
		printf("ch_th is only used for agcs only per ch load on NH, otherwise its value is meaningless\n");
		printf("For agcs only per ch load on NH, avg_th, rx_min_pkt, tx_min_pkt, sample_window, hit_cnt and nav_mitigation_th are meaningless\n");
	}
...
/**
 *  @brief process get_foundry_type
 *  @param argc  number of arguments
 *  @param argv  A pointer to arguments array
 *  @return  MLAN_STATUS_SUCCESS--success, otherwise--fail
 */
...
static int process_foundry_type(int argc, char *argv[])
{
	...
	if (ioctl(sockfd, MLAN_ETH_PRIV, &ifr)) {
		perror("mlanutl");
		fprintf(stderr, "mlanutl: fab code info fail\n");
		ret = MLAN_STATUS_FAILURE;
		goto done;
	}
	...
	if (argc == 3) {
		/* GET operation */
		memcpy(&fab_code, buffer, sizeof(t_s8));
		if (fab_code == 1)
			printf("Foundry_type is TSMC\n");

		else if (fab_code == 2)
			printf("Foundry_type is UMC\n");

		else
			printf("invalid foundry_type\n");
	}
